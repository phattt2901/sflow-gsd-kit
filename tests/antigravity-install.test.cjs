...
`;
  ...
  test('maps Claude tools to Antigravity tool names', () => {
    const result = convertClaudeAgentToAntigravityAgent(claudeAgent, false);
    // Read → view_file, Bash → run_command
    assert.ok(result.includes('view_file'), result);
    assert.ok(result.includes('run_command'), result);
  ...
  test('excludes Task tool (filtered by convertAntigravityToolName)', () => {
    const result = convertClaudeAgentToAntigravityAgent(claudeAgent, false);
    // Task is excluded by convertAntigravityToolName (returns null for Task)
    const fmEnd = result.indexOf('---', 3);
    const frontmatter = result.slice(0, fmEnd);
