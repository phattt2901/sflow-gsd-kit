...
function convertAntigravityToolName(claudeTool) {
  ...
  if (claudeTool === 'Task') {
    return null; // Agents are native or handled via browser_subagent
  }
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  // Brand conversion
  c = c.replace(/\bClaude Code\b/g, 'Antigravity');
  
  // Tool names in body text conversion
  c = c.replace(/\bBash\(/g, 'run_command(');
  c = c.replace(/\bRead\(/g, 'view_file(');
  c = c.replace(/\bWrite\(/g, 'write_to_file(');
  c = c.replace(/\bEdit\(/g, 'replace_file_content(');
  c = c.replace(/\bGlob\(/g, 'list_dir(');
  c = c.replace(/\bGrep\(/g, 'grep_search(');
  c = c.replace(/\bWebSearch\(/g, 'search_web(');
  c = c.replace(/\bWebFetch\(/g, 'read_url_content(');
...
function getAntigravityAdapterHeader(name) {
  return `<antigravity_adapter>
## A. Tool Usage for Antigravity
You are running within Antigravity. Use these tools:
- \`run_command\` for terminal execution
- \`view_file\`, \`list_dir\` for reading files/directories
- \`write_to_file\`, \`replace_file_content\` for modifications
- \`grep_search\` for searching codebase
- \`search_web\`, \`read_url_content\` for web research
- \`browser_subagent\` for advanced visual web navigation or authenticated tasks.

## B. Subagents & Tasks
Antigravity handles logic directly. If a workflow asks to spawn a subagent via \`Task()\`, you can execute the subagent's instructions natively within your own context, or utilize \`browser_subagent\` if visual web interaction is required.
</antigravity_adapter>`;
}
...
function convertClaudeCommandToAntigravitySkill(content, skillName, isGlobal = false) {
  ...
  const description = extractFrontmatterField(frontmatter, 'description') || '';
  const adapter = getAntigravityAdapterHeader(name);
...
function convertClaudeAgentToAntigravityAgent(content, isGlobal = false) {
  ...
  // Map tools to Antigravity equivalents
  const claudeTools = toolsRaw.split(',').map(t => t.trim()).filter(Boolean);
  const mappedTools = claudeTools.map(t => convertAntigravityToolName(t)).filter(Boolean);
  
  // Conditionally inject browser_subagent
  if (mappedTools.includes('search_web') || mappedTools.includes('read_url_content')) {
    if (!mappedTools.includes('browser_subagent')) {
      mappedTools.push('browser_subagent');
    }
  }
